...
def delivery_orders(request):
    ...
    # Iterate over each order assigned to the delivery boy
    for assignment in delivery_boy_orders:
        # Retrieve the last assigned status for the current order
        last_assignment = DeliveryBoyAssignment.objects.filter(order=assignment.order).latest('date')
        # Add the order ID and its last assigned status to the dictionary
        order_last_assigned_status[assignment.order.id] = last_assignment.status

    # Pass the fetched orders and last assigned statuses to the template for rendering
    return render(request, 'delivery_orders.html', {'delivery_boy_orders': delivery_boy_orders, 'order_last_assigned_status': order_last_assigned_status,'status' : last_assignment})
